...
$(window)
  .focus(function () {
    (window_focus = !0), $("#clickToPlay").fadeOut(300);
  })
  .blur(function () {
    (window_focus = !1), $("#clickToPlay").fadeIn(300);
  }),
  $(document).ready(function () {
    init(),
      ...
      $("#splitBlob").click(function () {
        blobColl.split();
      }),
      $("#joinBlob").click(function () {
        blobColl.join();
      }),
      $("#gravityBlob").click(function () {
        toggleGravity();
      }),
      $("#leftArrow").click(function () {
        blobColl.addForce(new Vector(-120, 0));
      }),
      $("#rightArrow").click(function () {
        blobColl.addForce(new Vector(120, 0));
      }),
      $("#upArrow").click(function () {
        blobColl.addForce(new Vector(0, -120));
      }),
...
function Environment(b, d, e, g) {
  (this.left = b),
    ...
    (this.collision = function (k) {
      ...
      return (
        k.getX() < this.left
          ? (k.setX(this.left), (m = !0))
          : k.getX() > this.right && (k.setX(this.right), (m = !0)),
        k.getY() < this.top
          ? (k.setY(this.top), (m = !0))
          : k.getY() > this.buttom && (k.setY(this.buttom), (m = !0)),
        m
      );
    }),
    (this.draw = function () {});
  ...
  (this.cur = new Vector(b, d)),
    ...
    (this.move = function (g) {
      ...
      (n = g * g),
        ...
        (k =
          (2 - this.friction) * m -
          (1 - this.friction) * this.prev.getX() +
          l * n),
        this.prev.setX(m),
        this.cur.setX(k),
        ...
        (k =
          (2 - this.friction) * m -
          (1 - this.friction) * this.prev.getY() +
          l * n),
        this.prev.setY(m),
        this.cur.setY(k);
    }),
    ...
    (this.getVelocity = function () {
      var g, k;
      return (
        (g = this.cur.getX() - this.prev.getX()),
        (k = this.cur.getY() - this.prev.getY()),
        g * g + k * k
      );
    }),
    (this.draw = function (g, k) {
      (g.lineWidth = 2),
        ...
        g.beginPath(),
        g.arc(this.cur.getX() * k, this.cur.getY() * k, 4, 0, 2 * Math.PI, !0),
        g.fill();
    });
}
...
function ConstraintY(b, d, e, g) {
  (this.pointMass = b),
    ...
    (this.sc = function () {
      var k;
      if (
        ((k = Math.abs(this.pointMass.getYPos() - this.y)),
        this.delta.setY(-k),
        0 != this.shortConst && k < this.shortConst)
      ) {
        var l;
        (l = this.shortConst / k),
          this.delta.scale(l),
          b.getPos().sub(this.delta);
...
function Stick(b, d) {
  (this.length = (function (g, k) {
    var l, m;
    return (
      (l = g.getXPos() - k.getXPos()),
      (m = g.getYPos() - k.getYPos()),
      Math.sqrt(l * l + m * m)
    );
  })(b, d)),
    ...
    (this.sc = function () {
      var k, l, m, n;
      (m = this.pointMassA.getPos()),
        (n = this.pointMassB.getPos()),
        this.delta.set(n),
        this.delta.sub(m),
        (k = this.delta.dotProd(this.delta)),
        (l = this.lengthSquared / (k + this.lengthSquared) - 0.5),
        this.delta.scale(l),
        m.sub(this.delta),
        n.add(this.delta);
    }),
    ...
    (this.draw = function (g, k) {
      this.pointMassA.draw(g, k),
        this.pointMassB.draw(g, k),
        (g.lineWidth = 3),
        (g.fillStyle = "#000000"),
        (g.strokeStyle = "#000000"),
        g.beginPath(),
        g.moveTo(this.pointMassA.getXPos() * k, this.pointMassA.getYPos() * k),
        g.lineTo(this.pointMassB.getXPos() * k, this.pointMassB.getYPos() * k),
        g.stroke();
    });
}
...
function Spring(b, d, e, g, k) {
  (this.restLength = b),
    ...
    (this.tmp = Vector(0, 0)),
    (this.sc = function (l) {
      l.collistion(this.pointMassA.getPos(), this.pointMassA.getPrevPos()),
        l.collistion(this.pointMassB.getPos(), this.pointMassB.getPrevPos());
    }),
    (this.move = function (l) {
      var m, n, o, q;
      (m = this.pointMassA.getXPos() - this.pointMassB.getXPos()),
        (n = this.pointMassA.getYPos() - this.pointMassB.getYPos()),
        (q = Math.sqrt(m * m + n * n)),
        (o = this.stiffness * (q / this.restLength - 1));
      var r, s, u;
      (r = this.pointMassA.getXVel() - this.pointMassB.getXVel()),
        (s = this.pointMassA.getYVel() - this.pointMassB.getYVel()),
        (u = r * m + s * n),
        (u *= this.damper);
      var z, A;
      (z = (o + u) * m),
        ...
        this.pointMassA.move(l),
        this.pointMassB.move(l);
    }),
    (this.addForce = function (l) {
      this.pointMassA.addForce(l), this.pointMassB.addForce(l);
    }),
    (this.draw = function (l, m) {
      this.pointMassA.draw(l, m),
        this.pointMassB.draw(l, m),
        (l.fillStyle = "#000000"),
        (l.strokeStyle = "#000000"),
        l.beginPath(),
        l.moveTo(this.pointMassA.getXPos() * m, this.pointMassA.getYPos() * m),
        l.lineTo(this.pointMassB.getXPos() * m, this.pointMassB.getXPos() * m),
        l.stroke();
    });
}
...
function Blob(b, d, e, g) {
  ...
  var m = 0.95,
    ...
    r;
  for (q = 0, o = 0; q < g; q++) {
    (this.pointMasses[q] = new PointMass(
      Math.cos(o) * e + b,
      Math.sin(o) * e + d,
      1
    )),
  ...
  for (
    this.middlePointMass = new PointMass(b, d, 1),
      this.pointMasses[0].setMass(4),
      this.pointMasses[1].setMass(4),
      q = 0;
    q < g;
    q++
  ) {
    this.sticks[q] = new Stick(
      this.pointMasses[q],
  ...
  (this.addBlob = function (s) {
    var u = this.joints.length,
      z;
    (this.joints[u] = new Joint(
      this.middlePointMass,
      s.getMiddlePointMass(),
      0,
      0
    )),
      (z = this.radius + s.getRadius()),
      this.joints[u].setDist(0.95 * z, 0);
  }),
    ...
    (this.sc = function (s) {
      var u, z;
      for (z = 0; 4 > z; z++) {
        for (u = 0; u < this.pointMasses.length; u++)
          !0 ==
          s.collision(
            this.pointMasses[u].getPos(),
            this.pointMasses[u].getPrevPos()
          )
            ? this.pointMasses[u].setFriction(0.75)
            : this.pointMasses[u].setFriction(0.01);
    ...
    (this.setForce = function (s) {
      var u;
      for (u = 0; u < this.pointMasses.length; u++)
        this.pointMasses[u].setForce(s);
      this.middlePointMass.setForce(s);
    }),
    (this.addForce = function (s) {
      var u;
      for (u = 0; u < this.pointMasses.length; u++)
        this.pointMasses[u].addForce(s);
      this.middlePointMass.addForce(s),
        this.pointMasses[0].addForce(s),
        this.pointMasses[0].addForce(s),
        this.pointMasses[0].addForce(s),
        this.pointMasses[0].addForce(s);
    }),
    (this.moveTo = function (s, u) {
      var z, A;
      for (
        A = this.middlePointMass.getPos(),
          s -= A.getX(s),
          u -= A.getY(u),
          z = 0;
        z < this.pointMasses.length;
        z++
      )
        (A = this.pointMasses[z].getPos()), A.addX(s), A.addY(u);
    ...
    (this.drawEars = function (s, u) {
      (s.strokeStyle = "#000000"),
        ...
        (s.lineWidth = 2),
        s.beginPath(),
        s.moveTo(-0.55 * this.radius * u, -0.35 * this.radius * u),
        s.lineTo(-0.52 * this.radius * u, -0.55 * this.radius * u),
        s.lineTo(-0.45 * this.radius * u, -0.4 * this.radius * u),
        s.fill(),
        s.stroke(),
        s.beginPath(),
        s.moveTo(0.55 * this.radius * u, -0.35 * this.radius * u),
    ...
    (this.drawHappyEyes2 = function (s, u) {
      (s.lineWidth = 1),
        ...
        (s.lineWidth = 1),
        s.beginPath(),
        s.moveTo(-0.25 * this.radius * u, -0.2 * this.radius * u),
        s.lineTo(-0.05 * this.radius * u, -0.2 * this.radius * u),
        s.stroke(),
        s.beginPath(),
        s.moveTo(0.25 * this.radius * u, -0.2 * this.radius * u),
        s.lineTo(0.05 * this.radius * u, -0.2 * this.radius * u),
    ...
    (this.drawHappyFace1 = function (s, u) {
      (s.lineWidth = 2),
        ...
        (s.fillStyle = "#000000"),
        s.beginPath(),
        s.arc(0, 0, 0.25 * this.radius * u, 0, Math.PI, !1),
        s.stroke();
    }),
    ...
    (this.drawOohFace = function (s, u) {
      (s.lineWidth = 2),
        ...
        (s.fillStyle = "#000000"),
        s.beginPath(),
        s.arc(0, 0.1 * this.radius * u, 0.25 * this.radius * u, 0, Math.PI, !1),
        s.fill(),
        s.beginPath(),
        s.moveTo(-0.25 * this.radius * u, -0.3 * this.radius * u),
        s.lineTo(-0.05 * this.radius * u, -0.2 * this.radius * u),
        s.lineTo(-0.25 * this.radius * u, -0.1 * this.radius * u),
        s.moveTo(0.25 * this.radius * u, -0.3 * this.radius * u),
        s.lineTo(0.05 * this.radius * u, -0.2 * this.radius * u),
        s.lineTo(0.25 * this.radius * u, -0.1 * this.radius * u),
        s.stroke();
    ...
    (this.drawFace = function (s, u) {
      1 == this.drawFaceStyle && 0.05 > Math.random()
        ? (this.drawFaceStyle = 2)
        : 2 == this.drawFaceStyle &&
          0.1 > Math.random() &&
          (this.drawFaceStyle = 1),
        1 == this.drawEyeStyle && 0.025 > Math.random()
          ? (this.drawEyeStyle = 2)
          : 2 == this.drawEyeStyle &&
            0.3 > Math.random() &&
            (this.drawEyeStyle = 1),
        4e-3 < this.middlePointMass.getVelocity()
          ? this.drawOohFace(s, u)
          : (1 == this.drawFaceStyle
              ? this.drawHappyFace1(s, u, 0, -0.3)
              : this.drawHappyFace2(s, u, 0, -0.3),
            1 == this.drawEyeStyle
              ? this.drawHappyEyes1(s, u, 0, -0.3)
              : this.drawHappyEyes2(s, u, 0, -0.3));
    }),
    (this.getPointMass = function (s) {
      return (
        (s += this.pointMasses.length),
        (s %= this.pointMasses.length),
        this.pointMasses[s]
      );
    }),
    (this.drawBody = function (s, u) {
      ...
      for (
        s.strokeStyle = "#000000",
          s.fillStyle = !0 == this.selected ? "#CCFFCC" : "#FFFFFF",
          s.lineWidth = 5,
          s.beginPath(),
          s.moveTo(
            this.pointMasses[0].getXPos() * u,
            this.pointMasses[0].getYPos() * u
          ),
          z = 0;
        z < this.pointMasses.length;
        z++
    ...
    (this.draw = function (s, u) {
      ...
      this.drawCube(s, u),
        ...
        s.translate(
          ...
          (this.middlePointMass.getYPos() - 0.35 * this.radius) * u
        ),
        (A = new Vector(0, -1)),
        (B = new Vector(0, 0)),
  ...
  (this.maxNum = g),
    ...
    (this.split = function () {
      ...
      if (this.numActive != this.maxNum) {
        for (n = this.blobs.length, k = 0; k < this.blobs.length; k++)
          null != this.blobs[k] && this.blobs[k].getRadius() > m
            ? ((m = this.blobs[k].getRadius()), (o = this.blobs[k]))
            : null == this.blobs[k] && (n = k);
        for (
          o.scale(0.75),
            q = new Blob(o.getXPos(), o.getYPos(), o.getRadius(), 8),
            k = 0;
          k < this.blobs.length;
          k++
        )
          null != this.blobs[k] &&
            (this.blobs[k].addBlob(q), q.addBlob(this.blobs[k]));
    ...
    (this.findSmallest = function (k) {
      ...
      for (n = 0; n < this.blobs.length; n++)
        n != k &&
          null != this.blobs[n] &&
          this.blobs[n].getRadius() < l &&
          ((m = n), (l = this.blobs[n].getRadius()));
      return m;
    }),
    (this.findClosest = function (k) {
      var m,
        ...
        u;
      for (
        s = this.blobs[k].getMiddlePointMass(), r = 0;
        r < this.blobs.length;
        r++
      )
        r != k &&
          null != this.blobs[r] &&
          ((u = this.blobs[r].getMiddlePointMass()),
          (o = s.getXPos() - u.getXPos()),
          (q = s.getYPos() - u.getYPos()),
          (n = o * o + q * q),
          n < l && ((l = n), (m = r)));
      return m;
    }),
    (this.join = function () {
      var k, l, o, q, r;
      1 == this.numActive ||
        ((k = this.findSmallest(-1)),
        (l = this.findClosest(k)),
        (o = this.blobs[k].getRadius()),
        (q = this.blobs[l].getRadius()),
        (r = Math.sqrt(o * o + q * q)),
        (this.blobs[k] = null),
        this.blobs[l].scale((0.945 * r) / q),
        this.numActive--);
    }),
    (this.selectBlob = function (k, l) {
      ...
      if (null == this.selectedBlob) {
        for (m = 0; m < this.blobs.length; m++)
          null != this.blobs[m] &&
            ((o = this.blobs[m].getMiddlePointMass()),
            (aXbX = k - o.getXPos()),
            (aYbY = l - o.getYPos()),
            (dist = aXbX * aXbX + aYbY * aYbY),
            dist < n &&
              ((n = dist),
              dist < 0.5 * this.blobs[m].getRadius() &&
                ((this.selectedBlob = this.blobs[m]),
                (r = {
                  x: aXbX,
                  y: aYbY,
                }))));
        return (
          null != this.selectedBlob && this.selectedBlob.setSelected(!0), r
        );
      }
    }),
    (this.unselectBlob = function () {
      null == this.selectedBlob ||
        (this.selectedBlob.setSelected(!1), (this.selectedBlob = null));
    }),
    ...
    (this.move = function (k) {
      var l;
      for (l = 0; l < this.blobs.length; l++)
        null != this.blobs[l] && this.blobs[l].move(k);
    }),
    (this.sc = function (k) {
      var l;
      for (l = 0; l < this.blobs.length; l++)
        null != this.blobs[l] && this.blobs[l].sc(k);
      null != this.blobAnchor && this.blobAnchor.sc();
    }),
    (this.setForce = function (k) {
      var l;
      for (l = 0; l < this.blobs.length; l++)
        if (null != this.blobs[l]) {
    ...
    (this.addForce = function (k) {
      ...
      for (l = 0; l < this.blobs.length; l++)
        null != this.blobs[l] &&
          this.blobs[l] != this.selectedBlob &&
          (this.tmpForce.setX(k.getX() * (0.75 * Math.random() + 0.25)),
...
function init() {
  ...
  return null == d.getContext
    ? void alert(
        "You need Firefox version 1.5 or higher for this to work, sorry."
      )
    : void ((document.onkeydown = function (e) {
        var g;
        switch (((g = null == e ? window.event.keyCode : e.keyCode), g)) {
          case 37:
            blobColl.addForce(new Vector(-50, 0));
            break;
          case 38:
            blobColl.addForce(new Vector(0, -50));
            break;
          case 39:
            blobColl.addForce(new Vector(50, 0));
            break;
      ...
      (document.onmousedown = function (e) {
        ...
        !0 != stopped &&
          ((g = b(e)),
          null == g || (selectOffset = blobColl.selectBlob(g.x, g.y)));
      }),
