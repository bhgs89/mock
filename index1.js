...
$(window)
    .focus(function() {
        window_focus = !0, $('#clickToPlay')
            .fadeOut(300)
    })
    .blur(function() {
        window_focus = !1, $('#clickToPlay')
            .fadeIn(300)
    }), $(document)
    .ready(function() {
        init(), $('body')
...
function ConstraintY(b, d, e, g) {
    this.pointMass = b, this.y = d, this.delta = new Vector(0, 0), this.shortConst = e, this.longConst = g, this.sc = function() {
        ...
        if (k = Math.abs(this.pointMass.getYPos() - this.y), this.delta.setY(-k), 0 != this.shortConst && k < this.shortConst) {
            var l;
            l = this.shortConst / k, this.delta.scale(l), b.getPos()
                .sub(this.delta)
        } else if (0 != this.longConst && k > this.longConst) {
            var l;
            l = this.longConst / k, this.delta.scale(l), b.getPos()
                .sub(this.delta)
        }
    }
}
...
function Blob(b, d, e, g) {
    ...
    this.addBlob = function(s) {
        ...
        var z, A;
        for (A = this.middlePointMass.getPos(), s -= A.getX(s), u -= A.getY(u), z = 0; z < this.pointMasses.length; z++) A = this.pointMasses[z].getPos(), A.addX(s), A.addY(u);
...
function BlobCollective(b, d, e, g) {
    this.maxNum = g, this.numActive = 1, this.blobs = [], this.tmpForce = new Vector(0, 0), this.selectedBlob = null, this.blobs[0] = new Blob(b, d, 0.4, 8), this.split = function() {
        ...
        if (null == this.selectedBlob) {
            for (m = 0; m < this.blobs.length; m++) null != this.blobs[m] && (o = this.blobs[m].getMiddlePointMass(), aXbX = k - o.getXPos(), aYbY = l - o.getYPos(), dist = aXbX * aXbX + aYbY * aYbY, dist < n && (n = dist, dist < 0.5 * this.blobs[m].getRadius() && (this.selectedBlob = this.blobs[m], r = {
                x: aXbX
                , y: aYbY
            })));
...
function init() {
    ...
    }, env = new Environment(0.2, 0.2, 2.6, 1.6), blobColl = new BlobCollective(1, 1, 1, 200), gravity = new Vector(0, 10), stopped = !1, timeout())
}
